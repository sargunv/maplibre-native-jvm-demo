...
namespace maplibre_jni
{
    ...
    void CameraOptionsConversions::init(JNIEnv *env)
    {
        ...
        jclass localClass = env->FindClass("org/maplibre/kmp/native/CameraOptions");
        if (!localClass)
        {
        ...
        // Cache field IDs
        centerField = env->GetFieldID(cameraOptionsClass, "center", "Lorg/maplibre/kmp/native/LatLng;");
        if (!centerField)
        {
            throw std::runtime_error("Could not find center field");
        }

        paddingField = env->GetFieldID(cameraOptionsClass, "padding", "Lorg/maplibre/kmp/native/EdgeInsets;");
        if (!paddingField)
        {
            throw std::runtime_error("Could not find padding field");
        }

        anchorField = env->GetFieldID(cameraOptionsClass, "anchor", "Lorg/maplibre/kmp/native/ScreenCoordinate;");
        if (!anchorField)
        {
        ...
        constructor = env->GetMethodID(cameraOptionsClass, "<init>",
                                       "(Lorg/maplibre/kmp/native/LatLng;Lorg/maplibre/kmp/native/EdgeInsets;Lorg/maplibre/kmp/native/ScreenCoordinate;Ljava/lang/Double;Ljava/lang/Double;Ljava/lang/Double;)V");
        if (!constructor)
        {
    ...
    jobject CameraOptionsConversions::create(JNIEnv *env, const mbgl::CameraOptions &cameraOptions)
    {
        ...
        if (center)
            env->DeleteLocalRef(center);
        if (padding)
            env->DeleteLocalRef(padding);
        if (anchor)
            env->DeleteLocalRef(anchor);
        if (zoom)
            env->DeleteLocalRef(zoom);
        if (bearing)
            env->DeleteLocalRef(bearing);
        if (pitch)
