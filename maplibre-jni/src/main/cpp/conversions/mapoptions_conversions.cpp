...
namespace maplibre_jni
{
    ...
    void MapOptionsConversions::init(JNIEnv *env)
    {
        ...
        jclass localClass = env->FindClass("org/maplibre/kmp/native/MapOptions");
        if (!localClass)
        {
        ...
        // Cache field IDs
        mapModeField = env->GetFieldID(mapOptionsClass, "mapMode", "Lorg/maplibre/kmp/native/MapMode;");
        if (!mapModeField)
        {
            throw std::runtime_error("Could not find mapMode field");
        }

        constrainModeField = env->GetFieldID(mapOptionsClass, "constrainMode", "Lorg/maplibre/kmp/native/ConstrainMode;");
        if (!constrainModeField)
        {
            throw std::runtime_error("Could not find constrainMode field");
        }

        viewportModeField = env->GetFieldID(mapOptionsClass, "viewportMode", "Lorg/maplibre/kmp/native/ViewportMode;");
        ...
        northOrientationField = env->GetFieldID(mapOptionsClass, "northOrientation", "Lorg/maplibre/kmp/native/NorthOrientation;");
        if (!northOrientationField)
        {
            throw std::runtime_error("Could not find northOrientation field");
        }

        sizeField = env->GetFieldID(mapOptionsClass, "size", "Lorg/maplibre/kmp/native/Size;");
        if (!sizeField)
        {
        ...
        constructor = env->GetMethodID(mapOptionsClass, "<init>",
                                       "(Lorg/maplibre/kmp/native/MapMode;Lorg/maplibre/kmp/native/ConstrainMode;Lorg/maplibre/kmp/native/ViewportMode;ZLorg/maplibre/kmp/native/NorthOrientation;Lorg/maplibre/kmp/native/Size;F)V");
        if (!constructor)
        {
    ...
    jobject MapOptionsConversions::create(JNIEnv *env, const mbgl::MapOptions &mapOptions)
    {
        ...
        // Create MapMode enum
        jclass mapModeClass = env->FindClass("org/maplibre/kmp/native/MapMode");
        jmethodID fromNative = env->GetStaticMethodID(mapModeClass, "fromNative", "(I)Lorg/maplibre/kmp/native/MapMode;");
        ...
        jclass constrainModeClass = env->FindClass("org/maplibre/kmp/native/ConstrainMode");
        fromNative = env->GetStaticMethodID(constrainModeClass, "fromNative", "(I)Lorg/maplibre/kmp/native/ConstrainMode;");
        ...
        jclass viewportModeClass = env->FindClass("org/maplibre/kmp/native/ViewportMode");
        fromNative = env->GetStaticMethodID(viewportModeClass, "fromNative", "(I)Lorg/maplibre/kmp/native/ViewportMode;");
        ...
        jclass northOrientationClass = env->FindClass("org/maplibre/kmp/native/NorthOrientation");
        fromNative = env->GetStaticMethodID(northOrientationClass, "fromNative", "(I)Lorg/maplibre/kmp/native/NorthOrientation;");
